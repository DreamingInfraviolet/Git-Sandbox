...
(function () {
  ...
  function Branch ( options ) {
    ...
    if ( options.parentCommit && options.parentBranch ) {
      ...
      var br = options.parentBranch;
      do {
        this.parentCommit = br.commits.slice(-1)[0];
        if ( this.parentCommit )
          break;
      } while ( br = br.parentBranch );
      this.parentBranch = options.parentBranch;
    } else {
  ...
  Branch.prototype.commit = function ( options ) {
    ...
    if ( !isFirstBranch && isPathBeginning ) {
      ...
      // Add start point
      if ( !this.startPoint && this.parentBranch ) {
    ...
    // We previously did not have a commit on this branch, and have added one.
    // All children of this branch that haven't yet made a commit should start from here.
    if(this.commits.length == 1) {
      for (var i = 0; i < this.parent.branches.length; ++i) {
        var branch = this.parent.branches[i];
        if (branch.parentBranch && branch.parentBranch.name === this.name)
          branch.parentCommit = commit;
      }
    }
  ...
  //Adapted from http://stackoverflow.com/a/3368118/6028770
  ...
  function drawTextBG ( context, x, y, text, color, font, angle, useStroke ) {
    ...
    if ( useStroke ) {
      context.beginPath();
      roundRect(context, -(width / 2) - 4, -(height / 2) + 2, width + 8, height + 2 );
